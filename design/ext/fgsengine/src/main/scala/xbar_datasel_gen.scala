...
class XBarMux4Hw_DataSel(val numBanks: Int=2, val numSubBanks: Int=8, val numSbWidth: Int=64,val addrWidth: Int=18,val numElems: Int=32,val numHw: Int=4) extends Module {
  ...
  val io = IO(new Bundle{
    ...
    val pop_count_sum_bank_sb_hw_stage1 = Output(Vec(numBanks, Vec(numSubBanks, Vec(numHw, UInt(elemIdWidth.W)))))
    val scatterinc_amount_sb_hw_stage5  = Output(Vec(numBanks*numSubBanks,Vec(numHw,UInt(bitsArb.W))))

  // for resp selection
    val ar_subbank_num_stage2_ele       = Input(Vec(numElems,UInt(log2Ceil(numSubBanks).W)))
    val ar_byte_num_stage2_ele          = Input(Vec(numElems,UInt(log2Ceil(numSbWidth/8).W)))
  ...
  val s2CLK=io.G3bankStallStage2CLK
  val s3CLK=io.G3bankStallStage3CLK
  ...
  val gateStage2  = WireInit(VecInit((io.G3bankStallStage2CLKEn).map{clken => g2GateEnable && clken})) //g2GateEnable && (io.G3bankStallStage2CLKEn)
  val gateStage3  = WireInit(VecInit((io.G3bankStallStage3CLKEn).map{clken => g2GateEnable && clken}))
  ...
  //matchVec := DontCare
    val matchVec = WireInit(VecInit(Seq.fill(numElems)(0.U(numElems.W))))
    for (i <- (numElems - 1) to 0 by -1) {
      for (j <- 0 until i) {
        val addrMatch = addr(i)(17, 7) === addr(j)(17, 7) &&
                      ar_byte_addr(i)=== ar_byte_addr(j)
        matchVec(i).bitSet(j.U, addrMatch)
      }
      matchVec(i).bitSet(i.U, true.B)
      for (n <- 1 until (numElems - i)) {
        matchVec(i).bitSet((i + n).U, matchVec(i + n)(i))
     }
   }
  matchVec
 }
 ...
 io.ar_data_grant_bank_sb_hw_stage2 <> ar_data_grant_bank_sb_hw_stage2
  ...
  for ( bank <- 0 until numBanks) {
    for ( subBank <- 0 until numSubBanks) {
      for ( hw <- 0 until numHw) {
        ...
        val xbarMuxSelBkSbk_s3   = xcff(elemIdWidth,io.G2generalCLK,localReset,enable,stage3Nxt           )
        val xbarMuxSelBkSbk_s4p1 = xcff(elemIdWidth,io.G2generalCLK,localReset,enable,xbarMuxSelBkSbk_s3  )
        val xbarMuxSelBkSbk_s4p2 = xcff(elemIdWidth,io.G2generalCLK,localReset,enable,xbarMuxSelBkSbk_s4p1)
        val xbarMuxSelBkSbk_s4p3 = xcff(elemIdWidth,io.G2generalCLK,localReset,enable,xbarMuxSelBkSbk_s4p2)
        val xbarMuxSelBkSbk_s4p4 = xcff(elemIdWidth,io.G2generalCLK,localReset,enable,xbarMuxSelBkSbk_s4p3)
        val xbarMuxSelBkSbk_s4   = xcff(elemIdWidth,io.G2generalCLK,localReset,enable,xbarMuxSelBkSbk_s4p4)
      
        val xbarMuxSelBkSbk_s5   = xcff(elemIdWidth,io.G3bankStallStage5CLK(bank),localReset,s5enable(bank),xbarMuxSelBkSbk_s4  )
...
val ar_ele_match_addr_vec = genMatchVec(io.ar_addr,io.ar_byte_addr)   //32(numElems) Vecs(Matches)
val sel = io.ar_addr_grant_sb_raw
val ar_winning_ele_match_vec_for_sb=VecInit(sel.map(sel_sb => Xtmux32(32,ar_ele_match_addr_vec,sel_sb.asBools.toSeq)))
...
for(i <- 0 until numElems)
   for(sbkIdx <- 0 until numSubBanks*numBanks){
        ar_ele_addr_granted_for_sb(i)(sbkIdx) := io.ar_req_addr_for_arb_sb(sbkIdx)(i).asBool &&
                                                 ar_winning_ele_match_vec_for_sb(sbkIdx)(i).asBool && 
                                                 (!(io.kill_ar_addr_grant_sb(sbkIdx)(i).asBool))  &&
                                                 (!(!io.ar_addr_age(i).asBool&&io.kill_age0_grant)) &&
                                                 !(io.ar_addr_age(i).asBool &&io.kill_age1_grant)
       ar_ele_addr_granted_for_sb_stage1(i)(sbkIdx) := xcff(1, io.G3eleStall1CLK(i),localReset,eleS1GateEnable(i), ar_ele_addr_granted_for_sb(i)(sbkIdx))
  }
    ...
    for(sbkId <- 0 until numSubBanks)
       for(hwId <- 0 until numHw)
          entry_elig_for_bank_sb_hw(eId)(bkId)(sbkId)(hwId) := ar_ele_addr_granted_for_sb_stage1(eId)(bkId*numSubBanks+sbkId) &&
                                                              (io.ar_byte_num_stage1_ele(eId)(2,1) === hwId.U(2.W)) &&
                                                              (io.any_scatter_valid_stage1_ele(eId) ||io.ar_inc16_stage1_ele(eId))
for( bkId <- 0 until numBanks)
   for( sbkId <- 0 until numSubBanks)
       for( hwId <- 0 until numHw)
       {
          ar_req_data_bank_sb_hw(bkId)(sbkId)(hwId) :=  Cat((Seq.tabulate(numElems) {i => entry_elig_for_bank_sb_hw(i)(bkId)(sbkId)(hwId)}).reverse)  //.reverse.reduce(Cat(_,_))
          //---------------------------------   
          //wire [5:0] ar_data_grant_bank1_sb7_hw3_stage1;
          val ar_data_grant_bank_sb_hw_stage1 = Wire(UInt(bitsArb.W))
               ar_data_grant_bank_sb_hw_stage1:= Xacc_lzc32(io.ar_addr_age_reverse_vec.asUInt,ar_req_data_bank_sb_hw(bkId)(sbkId)(hwId))
          ...
          always @(posedge G3bankStallStage2CLK1 or posedge MyReset) ar_data_grant_bank1_sb7_hw3_stage2_reg <= `XT_SEQ_DELAY MyReset ? 6'b0 :
           ...
           ar_data_grant_bank_sb_hw_stage2(bkId)(sbkId)(hwId):=xcff(bitsArb,s2CLK(bkId),localReset,gateStage2(bkId),ar_data_grant_bank_sb_hw_stage1)
         
          val ar_data_grant_bank_sb_hw_stage3= xcff(bitsArb,s3CLK(bkId),localReset,gateStage3(bkId),ar_data_grant_bank_sb_hw_stage2(bkId)(sbkId)(hwId))
...
//val  scatterinc_amount_sb_hw_stage5        =wire(Vec(numSubbanks*numBanks,UInt(6.W))) //[5:0]
  ...
  for(sbId <- 0 until numSubBanks*numBanks)
  {
    val bkId   = sbId / numSubBanks
    val _sbkId = sbId % numSubBanks
    //val s1CLK=io.
    ...
    for(hwId <- 0 until numHw )
    {
        val scatterinc_amount_sb_hw_stage1            = pop_count_sum_bank_sb_hw_stage1(bkId)(_sbkId)(hwId)
        val scatterinc_amount_sb_hw_stage2            = xcff(6,s2CLK(bkId),localReset,s2Enable,scatterinc_amount_sb_hw_stage1    )
        val scatterinc_amount_sb_hw_stage3            = xcff(6,s3CLK(bkId),localReset,s3Enable,scatterinc_amount_sb_hw_stage2    )
        val scatterinc_amount_sb_hw_stage4_p1         = xcff(6,g2CLK,localReset,g2Enable,scatterinc_amount_sb_hw_stage3    )
        val scatterinc_amount_sb_hw_stage4_p2         = xcff(6,g2CLK,localReset,g2Enable,scatterinc_amount_sb_hw_stage4_p1 )
        val scatterinc_amount_sb_hw_stage4_p3         = xcff(6,g2CLK,localReset,g2Enable,scatterinc_amount_sb_hw_stage4_p2 )
        val scatterinc_amount_sb_hw_stage4_p4         = xcff(6,g2CLK,localReset,g2Enable,scatterinc_amount_sb_hw_stage4_p3 )
        val scatterinc_amount_sb_hw_stage4            = xcff(6,g2CLK,localReset,g2Enable,scatterinc_amount_sb_hw_stage4_p4 )
        io.scatterinc_amount_sb_hw_stage5(sbId)(hwId):= xcff(6,g2CLK,localReset,g2Enable,scatterinc_amount_sb_hw_stage4    )  
     }
  }
